...
def content_preprocess(content: str) -> str | None:
    if content is None:
        return None
    # 去除文本中的 URL
    content = remove_url(content)
    content = clean_text(content)
    # lst = get_replacement_list(content)
    content = replace_variable(content)
    return trim(content)
...
def cross_validate_category(text: str, current_category: str, matched_keywords: List[str]) -> Tuple[str, List[str]]:
    """交叉验证分类结果，严格优先级：购物 > 运营商 > 银行"""
    text_lower = text.lower()
    text_upper = text.upper()  # 用于机构关键词匹配

    # 1. 检查是否包含购物机构关键词（最高优先级）
    shopping_institution_matches = []
    for shop in SHOPPING_INSTITUTIONS:
        pattern = r'\b' + re.escape(shop) + r'\b'
        if re.search(pattern, text_upper):
            shopping_institution_matches.append(shop)

    if shopping_institution_matches:
        return '购物', shopping_institution_matches + matched_keywords

    # 2. 检查是否包含运营商机构关键词（第二优先级）
    operator_institution_matches = []
    for operator in OPERATOR_INSTITUTIONS_EXTENDED:
        pattern = r'\b' + re.escape(operator) + r'\b'
        if re.search(pattern, text_upper):
            operator_institution_matches.append(operator)

    if operator_institution_matches:
        return '运营商', operator_institution_matches + matched_keywords

    # 3. 检查是否包含银行机构关键词（第三优先级）
    bank_institution_matches = []
    for bank in BANK_INSTITUTIONS:
        pattern = r'\b' + re.escape(bank) + r'\b'
        # 特殊处理MB银行，避免与MobiFone混淆
        if bank == 'mb':
            if re.search(pattern, text_lower) and \
                    not any(exclude in text_lower for exclude in ['mobifone', 'mobiedu', 'combo khóa học']):
                bank_institution_matches.append(bank)
        elif re.search(pattern, text_lower):
            bank_institution_matches.append(bank)

    if bank_institution_matches:
        return '银行', bank_institution_matches + matched_keywords
...
def detect_primary_category(text: str, sender: str = "") -> Tuple[str, List[str]]:
    """改进的一级分类检测函数，使用树状优先级匹配"""
    """改进的一级分类检测函数，使用树状优先级匹配，考虑发送方"""
    # 首先根据发送方分类
    if sender:
    ...
    loan_financial_terms = [
        ...
        'tín dụng', 'tin dung', 'hạn mức', 'han muc', 'duyệt hồ sơ', 'duyet ho so',
    ...
    # 检查贷款相关关键词
    for term in loan_financial_terms + loan_progress_terms:
        if re.search(r'\b' + re.escape(term) + r'\b', text_lower):
            matches.append(term)
    for loan in loan_keywords:
        if re.search(r'\b' + re.escape(loan) + r'\b', text_lower):
            matches.append(loan)
    ...
    for indicator in bank_transaction_indicators:
        if re.search(r'\b' + indicator + r'\b', text_lower):
            bank_transaction_matches.append(indicator)

    if bank_transaction_matches:
        category, final_matches = '银行', bank_transaction_matches
        return cross_validate_category(text, category, final_matches)
    ...
    for bank in bank_keywords:
        ...
        if bank == 'mb':
            if re.search(pattern, text_lower) and \
                    not any(exclude in text_lower for exclude in ['mobifone', 'mobiedu', 'combo khóa học']):
                bank_name_matches.append(bank)
        elif bank == 'shinhan':
            if re.search(pattern, text_lower) and \
                    'shinhanlife' not in text_lower:
                bank_name_matches.append(bank)
        else:
    ...
    if bank_name_matches:
        category, final_matches = '银行', bank_name_matches
        return cross_validate_category(text, category, final_matches)
    ...
    for keyword in shopping_keywords:
        if re.search(r'\b' + re.escape(keyword) + r'\b', text_lower):
            shopping_matches.append(keyword)

    if shopping_matches:
        category, final_matches = '购物', shopping_matches
        return cross_validate_category(text, category, final_matches)
    ...
    operator_matches = []
    for indicator in operator_specific_indicators:
        if re.search(r'\b' + re.escape(indicator) + r'\b', text_lower):
            operator_matches.append(indicator)

    if operator_matches:
        category, final_matches = '运营商', operator_matches
        return cross_validate_category(text, category, final_matches)
